...
public interface IRoleInTerrainPlaceholder<O, P extends O> extends TerrainPlaceholderFormatter<O, P> {
    @Override
    default @NotNull String name() {
        return "role-in-terrain";
    }
    ...
    default String formatPlaceholder(@Nullable O player, @NotNull String params, @Nullable Terrain terrain) {
        LanguageHolder<?, ?> lang = Terrainer.lang();
        if (terrain == null) return lang.get("Placeholder Values.Unknown Terrain");
        UUID uuid = player == null ? null : uuid(player);
        ...
        if (Objects.equals(uuid, terrain.owner())) {
            ...
            return lang.get("Placeholder Values.Roles.Unrelated");
        }
    }
}
